...
public class MovieServiceTest {
    ...
    public void testGetAllMovies() throws Exception  {
        movieService.save(createTestMovie()).get();
    ...
    public void testDeleteNonExistentMovie() {
        // Attempt to delete the non-existent movie
        try {
            movieService.delete(12345L).get();
            fail("Expected an exception to be thrown");
        } catch (Exception e) {
            // Check if the exception is the expected type
            assertThat(e, is(instanceOf(NotFoundException.class)));
    ...
    public void testMissingRequiredValue() {
        Movie movie = createTestMovie();
        movie.setTitle(null); // Remove the title
        // Attempt to save the movie
        try {
